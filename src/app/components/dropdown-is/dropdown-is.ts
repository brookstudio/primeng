...
export const DROPDOWN_IS_VALUE_ACCESSOR: any = {
  provide: NG_VALUE_ACCESSOR,
  useExisting: forwardRef(() => DropdownIS),
  multi: true
};
...
@Component({
           ...
           <div #container [ngClass]="{'ui-dropdown ui-widget ui-state-default ui-corner-all ui-helper-clearfix':true,
              ...
              <div class="ui-helper-hidden-accessible" *ngIf="autoWidth">
                  <select [required]="required" name="options" [attr.aria-label]="selectedOption ? selectedOption.label : ' '" tabindex="-1" aria-hidden="true">
                      <option *ngFor="let option of optionsToDisplay | slice:0:limit;" [value]="option.value" [selected]="selectedOption == option">{{option.label}}</option>
                  </select>
              </div>
              ...
              <div #panel [ngClass]="'ui-dropdown-panel ui-widget-content ui-corner-all ui-helper-hidden ui-shadow'" [@panelState]="panelVisible ? 'visible' : 'hidden'"
                  ...
                  <div #itemswrapper class="ui-dropdown-items-wrapper" [style.max-height]="scrollHeight||'auto'">
                      <ul class="ui-dropdown-items ui-dropdown-list ui-widget-content ui-widget ui-corner-all ui-helper-reset" *ngIf="lazy ? panelVisible : true">
                          <li *ngFor="let option of optionsToDisplay | slice:0:limit;let i=index"
  ...
  providers: [DomHandler, ObjectUtils, DROPDOWN_IS_VALUE_ACCESSOR]
})
export class DropdownIS implements OnInit, AfterViewInit, AfterContentInit, AfterViewChecked, OnDestroy, ControlValueAccessor {
  ...
  @Input() createNewForInput: boolean = false;
  ...
  ngAfterViewInit() {
    ...
    if (this.appendTo) {
      ...
      else this.domHandler.appendChild(this.panel, this.appendTo);
    }
  }
  ...
  alignPanel() {
    if (this.appendTo) this.domHandler.absolutePosition(this.panel, this.container);
    else this.domHandler.relativePosition(this.panel, this.container);
  }
  ...
  activateFilter() {
    ...
    if (this.options && this.options.length) {
      ...
      if (!this.optionsToDisplay.find(item => item.label === this.filterValue) && this.createNewForInput) {
        if (this.optionsToDisplay) {
          this.optionsToDisplay.splice(0, 0, {
            label: this.filterValue + ` <span class="new-drop-down-item">(Create new one)</span>`,
  ...
  applyFocus(): void {
    if (this.editable) this.domHandler.findSingle(this.el.nativeElement, '.ui-dropdown-label.ui-inputtext').focus();
    else this.domHandler.findSingle(this.el.nativeElement, 'input[readonly]').focus();
  }
...
@NgModule({
  ...
  exports: [DropdownIS, SharedModule],
  declarations: [DropdownIS]
})
export class DropdownISModule {}
